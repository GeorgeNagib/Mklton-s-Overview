...
export default function Projects() {
  const [limitOfProjects, setLimitOfProjects] = useState(4)
  const { isLoading, isError, data, error } = useQuery(['projects', limitOfProjects], projectsService)

    return (
        
      <section className="projects pd-2-r">
        <div className="container">
          <h2 className="title-heading-2 projects">current projects</h2>
          ...
          {data?.projects?.sentProjects?.map(({_id, name, description}) => (
            <Project key={_id} name={name} description={description}></Project>
          ))}
          ...
          {
            data?.allProjectsLength > limitOfProjects || isLoading ? <LoadMore changeLimit={setLimitOfProjects} limit={limitOfProjects}></LoadMore> : null
          }
